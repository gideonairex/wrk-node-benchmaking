...
function parseExec () {
	...
	var reqPath = './lib/request.lua';
	// path for the request LUA template
	var reqTpl = './lib/requestTpl.txt';
	...
	glob( "benchmark/**/*.js", options, function ( err, files ) {
		if( err ) {
		...
		files.forEach( function ( fileDir, index ) {
			...
			fs.readFile( reqTpl, 'utf8', function ( err, data ) {
				...
				var headers           = {};
				headers.contentType   = luaOpt.customeHeaders.contentType || config.headers.contentType;
				headers.engineSecret  = luaOpt.customeHeaders.engineSecret || config.headers.engineSecret;
				headers.engineId      = luaOpt.customeHeaders.engineId || config.headers.engineId;
				headers.authorization = luaOpt.customeHeaders.authorization || config.headers.authorization;
